...
export default function AboutMe() {
  ...
  useEffect(() => {
    const updateDuration = () => setDuration(window.innerWidth < 768 ? 15 : 30);
    updateDuration();
  ...
  const itemVariants = { hidden: { opacity: 0, y: 40 }, visible: { opacity: 1, y: 0, transition: { duration: 1.5, ease: "easeInOut" } } };

  return (
    <motion.section className="max-w-[1200px] mx-auto w-full h-auto pb-4" initial="hidden" animate="visible" variants={containerVariants}>
      <AnimatedGradient>
        <motion.div className="md:flex md:w-full md:h-full" variants={containerVariants}>

          <motion.div variants={itemVariants} className="md:w-1/2 md:h-full">
            <img src="/images/satellite-headshot-resized.jpeg" alt="Satellite Nongmaithem" className="md:w-full md:h-full object-cover rounded-3xl shadow-xl" />
          </motion.div>

          <motion.div variants={itemVariants} className="md:w-1/2 flex flex-col justify-center p-8 gap-6 text-left">
        ...
        <div className="mt-6 overflow-hidden w-full">
          <h4 className="text-lg font-semibold mb-3 mt-2 text-[var(--color-secondary)] text-center">Software & Languages</h4>
          <motion.div className="flex gap-6 whitespace-nowrap" animate={{ x: ["0%", "-100%"] }} transition={{ repeat: Infinity, duration, ease: "linear" }}>
            {tools.concat(tools).map((tool, i) => (
              <div key={i} className="flex items-center gap-2 min-w-[120px] md:min-w-[140px]">
